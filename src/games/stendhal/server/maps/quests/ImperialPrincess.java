...
public class ImperialPrincess extends AbstractQuest {
	 ...
	 */
	private static final int POTION_DIVISOR = 3;
	
	/** The player is asked to get a number of herbs depending on level. 
	 * So if they are level 40, they must bring 2 + 1 antidotes
	 */
	private static final int ANTIDOTE_DIVISOR = 4;
	...
	public List<String> getHistory(final Player player) {
		...
		if (player.isQuestInState(QUEST_SLOT, "recommended", "done")) {
			res.add("Przyniosłem Princess Ylflia wszystkie składniki potrzebne do uzdrawiania. Powiedziała mi, że poleci mnie swemu ojcu, królowi.");
		}
						...
						try {
										...
										required_antidote))
							{
								...
								raiser.say("Doskonale! Zarekomenduję Ciebie mojemu ojcu jako dobrą " +
										...
										"obywatela Kalavan.");
								player.addXP(level * 2000);
								player.addatk_xp(level * 500);
								player.adddef_xp(level * 400);
	...
	public void addToWorld() {
		...
		fillQuestInfo(
				...
				"Jeżeli chcesz dostać oficjalne obywatelstwo miasta Kalavan to najpierw musisz zapytać o pozwolenie króla nim porozmawiasz z jego córką Princess Ylflia...",
				true);
		step_1();
		step_2();
		step_3();
