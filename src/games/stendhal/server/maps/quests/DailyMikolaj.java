/* $Id: DailyMikolaj.java,v 1.80 2023/11/27 23:13:16 davvids Exp $ */
/***************************************************************************
...
public class DailyMikolaj extends AbstractQuest {

	private static final String QUEST_SLOT = "daily_santa";
	private final SpeakerNPC npc = npcs.get("Święty Mikołaj");
	private static Logger logger = Logger.getLogger("DailyMikolaj");
	...
	static class DailyQuestAction implements ChatAction {
		...
		public void fire(final Player player, final Sentence sentence, final EventRaiser raiser) {
			...
			if (player.isBadBoy()) {
				...
				raiser.say("Potrzebuje pomocy ze stworami nawiedzającymi naszą świąteczną krainę. Idź zabij " + Grammar.a_noun(creatureName)
						+ " w imieniu Świętego Mikołaja i powiedz #załatwione, gdy skończysz.");
	...
	public List<String> getHistory(final Player player) {
		...
		res.add("Odwiedziłem Świętego Mikołaja");
		final String questState = player.getQuest(QUEST_SLOT);
		if ("rejected".equals(questState)) {
			res.add("Nie chcę pomóc Świętemu Mikołajowi.");
			return res;
		}

		res.add("Chcę pomóc Świętemu Mikołajowi.");
		if (player.hasQuest(QUEST_SLOT) && !player.isQuestCompleted(QUEST_SLOT)) {
			...
			if (!questDone) {
				res.add("Zostałem poproszony o zabicie" + creatureToKill
						+ ", aby pomóc Świętemu Mikołajowi. Jeszcze go nie zabiłem.");
			} else {
				res.add("Zabiłem " + creatureToKill
						+ ", aby pomóc Świętemu Mikołajowi.");
			}
		}
		if (player.isQuestCompleted(QUEST_SLOT)) {
			...
			if (timeRemaining > 0L) {
				res.add("Zabiłem ostatniego potwora o którego mnie prosił Święty Mikołaj odebrałem nagrodę w ciągu 24 godzin.");
			} else {
				res.add("Zabiłem ostatniego potwora o którego mnie prosił Święty Mikołaj, teraz znów potrzebuje mojej pomocy.");
			}
		}
		...
		npc.add(ConversationStates.ATTENDING,
				...
				new ChatAction() {
					public void fire(Player player, Sentence sentence, EventRaiser npc) {
						npc.say("Już dostałeś zadanie na zgładzenie w imieniu Świętego Mikołaja " + 
				...
				new ChatAction() {
					public void fire(Player player, Sentence sentence, EventRaiser npc) {
						if(player.getQuest(QUEST_SLOT, 0)!=null) {
								npc.say("Już otrzymałeś zadanie na zgładzenie w imieniu Świętego Mikołaja " + 
										Grammar.a_noun(player.getQuest(QUEST_SLOT, 0).split(",")[0]) + 
										". Powiedz #załatwione kiedy to zrobisz!" +
	...
	private void step_3() {
				...
				new ChatAction() {
					public void fire(Player player, Sentence sentence, EventRaiser npc) {
							...
							npc.say("Jeszcze nie zabiłeś " + Grammar.a_noun(questKill)
									+ "w imieniu Świętego Mikołaja. Idź i zrób to i powiedz #załatwione, gdy skończysz.");
					}
				});
		...
		npc.add(ConversationStates.ATTENDING,
				...
				new AndCondition(
				        ...
				        new KilledForQuestCondition(QUEST_SLOT, 0)),
				ConversationStates.ATTENDING, 
				"Gratuluje! Pozwól mi podziekować! Weź te napitki na chłodne dni...",
				new MultipleActions(
						new IncreaseXPDependentOnLevelAction(5, 45.0),
						new IncreaseKarmaAction(5.0),
						new IncreaseAtkXPAction(1000),
						new IncreaseDefXPAction(1000),
						new EquipItemAction("napój zimowy 2023", 20),
	...
	public void addToWorld() {
		super.addToWorld();
		fillQuestInfo(
				"Zadanie u Świętego Mikołaja",
				"Święty Mikołaj potrzebuje kogoś zabić...",
				true);
		step_1();
		step_2();
		step_3();
