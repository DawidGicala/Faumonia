...
public class IcecreamForAnnie extends AbstractQuest {
	...
	public List<String> getHistory(final Player player) {
        ...
        if ("mummy".equals(questState) || isCompleted(player)) {
            res.add("Rozmawiałem z panią Jones, zgodziła się, teraz mogę dać lody jej córce.");        
        }
        if (isCompleted(player)) {
            if (isRepeatable(player)) {
                res.add("Dałem lody do Annie, ona dała mi prezent. Być może mam ochotę na jeszcze jeden prezent.");
            } else {
                res.add("Annie jest zajęta jedzeniem lodów, które jej dałem,. W zamian za lody dała mi prezent.	W nagrodę za ukończenie zadania otrzymałem: 500 pkt doświadczenia, 10 pkt karmy i prezent.");
            }			
		}
		return res;
	}
