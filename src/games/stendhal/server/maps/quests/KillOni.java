/* $Id: KillOni.java,v 1.32 2020/12/06 21:21:19 davvids Exp $ */
/***************************************************************************
 ...
 */
 ...
 public class KillOni extends AbstractQuest {

	private static final String QUEST_NPC = "Mid";
	private static final String QUEST_SLOT = "kill_oni";
	...
	public KillOni() {
		...
		enemyForces.put("oni",
				new Pair<Integer, String>(2000,"Zapomniałeś co mi obiecałeś? Miałeś wybrać się do pałacu na wschód od miasta i pozabijać Oni."));
	...
	class GiveQuestAction implements ChatAction {
		...
		public void fire(final Player player, final Sentence sentence, final EventRaiser speakerNPC) {
			...
			speakerNPC.say("Na wschód od miasta zlokalizowany jest pałac Oni. Tamtejsi mieszkańcy powinni nam płacić za ochronę lecz od miesięcy tego nie robią... Potrzebujemy kogoś aby dał im nauczkę i rozprawił się z oddziałami  " + monstersType +
	...
	class RewardPlayerAction implements ChatAction {
		...
		public void fire(final Player player, final Sentence sentence, final EventRaiser speakerNPC) {
			...
			if(killed == killsnumber) {
				...
				speakerNPC.say("Dobra robota! Weź tę eliksiry tojadowe jako zapłatę... Myśle, że za tydzień będe miał dla ciebie kolejne zadanie.");
			} else {
				// player killed more then needed soldiers
				speakerNPC.say("Bardzo dobrze! Zabiłeś "+(killed-killsnumber)+" extra "+
						Grammar.plnoun(killed-killsnumber, "soldier")+"! Weź tę eliksiry tojadowe jako zapłątę. Myśle, że za tydzień będe miał dla ciebie kolejne zadanie.");
			}
			int karmabonus = 441*(1*killed/(killsnumber)-1);
			final StackableItem money = (StackableItem)
					SingletonRepository.getEntityManager().getItem("eliksir tojadowy");
			...
			final String enemy = creatures.get(i);
		      ...
		      npc.add(ConversationStates.ATTENDING,
		    		  ...
		    		  new MultipleActions(
		    				  ...
		    				  new IncreaseXPAction(3000000),
							  new IncreaseAtkXPAction(40000),
							  new IncreaseDefXPAction(60000),
							  new EquipItemAction("bryłka złota", 60),
							  new EquipItemAction("bryłka mithrilu", 30),
	...
	public void addToWorld() {
		...
		fillQuestInfo(
				"Pałac Oni",
				"Mieszkańcy pałacu Oni, zlokalizowanego na wschód od Kirdneh nie chcą zapłacić za ochronę. Ktoś musi dać im nauczkę!",
				true);
		step_1();
	}
	...
	public boolean isRepeatable(final Player player) {
 	...
 	public List<String> getHistory(final Player player) {
		...
		if(player.getQuest(QUEST_SLOT, 0).equals("start")) {
			...
			history.add("Mid poprosił mnie o zabicie armii "+
					givenNumber+" "+
			...
			if(new KilledInSumForQuestCondition(QUEST_SLOT, 2, givenNumber).fire(player, null, null)) {
				history.add("Zabiłem wystaczająco oni, aby dostać moją nagrodę.");
			} else {
		...
		if(isCompleted(player)) {
			history.add("Pomogłem Midowi i otrzymałem zapas bryłek złota, mithrilu i eliksirów tojadowych!");
		}	
		if (isRepeatable(player)) {
			history.add("Mid znowu potrzebuje pomocy.");
		} 
		int repetitions = player.getNumberOfRepetitions(getSlotName(), 3);
		if (repetitions > 0) {
			history.add("Pokonałem całą armię "
					+ Grammar.quantityplnoun(repetitions, "") + " dla Mida z ratuszu Kirdneh.");
		}
		return history; 
 	}
