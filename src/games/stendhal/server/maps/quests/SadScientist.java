...
public class SadScientist extends AbstractQuest {
	...
	private static final String NEEDED_ITEMS = "szmaragd=50;szafir=30;rubin=20;sztabka złota=10;sztabka mithrilu=5";
	...
	public List<String> getHistory(final Player player) {
		...
		if ("rejected".equals(questState)) {
			res.add("Vasi Elos z zamku Kalavan poprosił mnie o pomoc, ale nie jestem zainteresowany, aby pomóc naukowcowi.");
			return res;
		} 
		res.add("Vasi Elos poprosił mnie, abym dostarczył mu kryształy, złoto i mithril, aby mógł sprawić dla swej słodkiej Very spodnie wysadzane klejnotami jako prezent.");
		...
		res.add("Vasi Elos potrzebuje spodni mainiocyjskich. Muszę mu je dostarczyć.");
		...
		res.add("Vasi Elos wysłał mnie do burmistrza Mayora Sakhsa, abym dowiedział się gdzie jest Vera.");
		...
		res.add("Vasi Elos jest naprawdę zrozpaczony. Naznaczył spodnie krwią swego brata.");
		if (questState.startsWith("decorating")) {
			return res;
		} 
		res.add("Spodnie naznaczone krwią są przepiękne. Należą teraz do mnie." +
				"Ale jakim kosztem.");
	...
	private void playerReturnsToFetchReward(SpeakerNPC npc) {
		...
		final ChatAction action = new MultipleActions(
											...
											new EquipItemAction("*spodnie z białego złota*", 1, true)
										);
		npc.add(ConversationStates.IDLE, 
				ConversationPhrases.GREETING_MESSAGES, 
				condition, 
				ConversationStates.IDLE, 
				"Oto gotowe *spodnie z białego złota*. Możesz je teraz nosić, jest na nich symbol mojego bólu. Bądź pozdrowiony",
				action);
	...
	private void playerReturnsAfterKillingTheImperialScientist(SpeakerNPC npc) {
		...
		npc.add(ConversationStates.ATTENDING, "symbol",
				...
				"Mam zamiar stworzyć parę przepięknych spodni. Wróć za 5 minut.", 
				action);
	}
	...
	private void playerReturnsWithLetter(final SpeakerNPC npc) {
		...
		npc.add(ConversationStates.INFORMATION_2, Arrays.asList("letter", "yes", "note", "list", "karteczka", "tak"),
				condition,
				ConversationStates.ATTENDING,
				"Och nie! Nie widzisz, że cierpię. W tej chwili nie jestem w stanie wykonać tych przepięknych spodni. " +
					...
					new CollectRequestedItemsAction(
							...
							new MultipleActions(
									...
									new SayTextAction("Ale jestem roztrzepany. Bezgraniczna miłość do mej żony Very kiedyś mnie zgubi. Potrzebuję jakieś spodnie aby te klejnoty do czegoś przymocować. " +
											"Proszę zdobądź spodnie mainiocyjskie. Dowidzenia.")), ConversationStates.IDLE
							));
		}
	}
...
//									new SetQuestAction(QUEST_SLOT,"making;"), new SetQuestToTimeStampAction(QUEST_SLOT, 1),
	...
	final AndCondition nolegscondition = new AndCondition(new GreetingMatchesNameCondition(npc.getName()),
									...
									new NotCondition(new PlayerHasItemWithHimCondition("spodnie mainiocyjskie"))
									);
	npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
			nolegscondition,
			ConversationStates.IDLE, 
			"Witaj ponownie. Proszę wróć gdy zdobędziesz spodnie mainiocyjskie, jako podstawę do wykonania spodni nabijanych klejnotami dla mojej żony Very.",
			null);		
		
	//player returns with legs
	final AndCondition legscondition = new AndCondition(new GreetingMatchesNameCondition(npc.getName()),
								new QuestInStateCondition(QUEST_SLOT, "legs"),
								new PlayerHasItemWithHimCondition("spodnie mainiocyjskie")
								);
	final ChatAction action = new MultipleActions(
	new SetQuestAction(QUEST_SLOT,"making;"),
	new SetQuestToTimeStampAction(QUEST_SLOT, 1),
	new DropItemAction("spodnie mainiocyjskie"));
	npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
			legscondition,
			ConversationStates.IDLE, 
			"Masz spodnie mainiocyjskie! Wspaniale! Niezwłocznie przystąpię do pracy. Zrobię je w bardzo krótkim czasie dzięki nowej technologi! " +
			"Proszę wróci za 3 godziny.",
			action);
	}
	...
	private void startOfQuest(final SpeakerNPC npc) {
		...
		npc.add(ConversationStates.QUEST_STARTED,
				...
				null,
				ConversationStates.QUEST_STARTED,
				"Do spodni wysadzanych klejnotami potrzebny jest szafir, rubin, 2 szmaragdy i 5 sztabek złota." +
