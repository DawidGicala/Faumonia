...
public class RedStone extends AbstractQuest {
	...
	public List<String> getHistory(final Player player) {
		...
		if ("rejected".equals(questState)) {
			res.add("Nie chcę zwoju od Miriri");
			return res;
		}
		res.add("Chcę zwój od Miriri");
		if ((player.isEquipped("money", REQUIRED_WOOD)) || isCompleted(player)) {
			res.add("Musze przynieść pieniadze.");
		}
		if (isCompleted(player)) {
			res.add("Dałem Miriri pieniądze. Dostałem przepiękny zwój");
		}
		if(isRepeatable(player)){
			res.add("Mogę iść po nowy zwój do Miriri.");
		} 
		return res;
	}
	...
	private void prepareRequestingStep() {
		...
		npc.add(ConversationStates.IDLE, 
			...
			"Dobrze, że jesteś. Przynieś szybko te 500 money a dostaniesz zwój!",
			null);
		...
		npc.add(ConversationStates.ATTENDING,
			...
			ConversationStates.ATTENDING,
			"Już się umówiliśmy, że dotarczysz pieniądze. 500 money, pamiętasz?",
			null);
		...
		npc.add(ConversationStates.ATTENDING,
				...
				"Mam przepiękny #zwój. Moge ci go dać ale nie ma nic za darmo. Przynieś mi 500 money a dam ci zwój. Co ty na to ?",
				null);
		...
		npc.add(ConversationStates.ATTENDING,
				...
				ConversationStates.QUEST_OFFERED, 
				"Mam kolejny zwój. Dam ci go za kolejne 500 money. Pasuje?",
				null);
	...
	private void prepareBringingStep() {
		...
		reward.add(new EquipItemAction("czerwony zwój", 1));
	...
	public void addToWorld() {
		...
		fillQuestInfo(
				"Czerwony zwój", 
				"Miriri da mi czerwony zwój za trochę złota.", 
				true);
