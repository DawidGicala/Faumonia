/* $Id: KillHighDwarfs.java,v 1.32 2020/12/07 15:52:19 davvids Exp $ */
/***************************************************************************
 ...
 */
 ...
 public class KillHighDwarfs extends AbstractQuest {

	private static final String QUEST_NPC = "Frank";
	private static final String QUEST_SLOT = "kill_high_dwarfs";
	...
	public KillHighDwarfs() {
		super();
		// fill monster types map
		enemyForces.put("górskich krasnali",
				new Pair<Integer, String>(10000,"Zapomniałeś co mi obiecałeś? Miałeś wybrać się w góry, odnaleźć opuszczoną kryptę i wybić górskich krasnali!"));
	...
	class GiveQuestAction implements ChatAction {
		...
		public void fire(final Player player, final Sentence sentence, final EventRaiser speakerNPC) {
			...
			speakerNPC.say("Krasnale mieszkające w górach regularnie niszczą nasze tunele w kopalni. Szukamy kogoś kto o wybierze się w góry, odnajdzie ich siedlisko i zabije grupę " + monstersType +
					" Zabij co najmniej " + enemyForces.get(monstersType).first()+
					" "+ monstersType +
					" a wynagrodzę cię. Powinieneś szukać ich w opuszczonej krypcie w zachodniej części gór.");
	...
	class RewardPlayerAction implements ChatAction {
		...
		public void fire(final Player player, final Sentence sentence, final EventRaiser speakerNPC) {
			...
			int moneyreward = 8*Rand.roll1D6();
			if(killed == killsnumber) {
				// player killed no more no less then needed soldiers
				speakerNPC.say("Świetnie! Nasze kopalnie na razie powinny być bezpieczne... weź te kamienie szlachetne z kopalni w ramach podziękowania. Myśle, że za tydzień będe miał dla ciebie kolejne zadanie.");
			} else {
				// player killed more then needed soldiers
				speakerNPC.say("Świetnie Zabiłeś "+(killed-killsnumber)+" więcej "+
						Grammar.plnoun(killed-killsnumber, "")+"! Nasze kopalnie na razie powinny być bezpieczne... weź te kamienie szlachetne z kopalni w ramach podziękowania. Myśle, że za tydzień będe miał dla ciebie kolejne zadanie.");
			}
	...
	class ExplainAction implements ChatAction {
		...
		public void fire(Player player, Sentence sentence, EventRaiser npc) {
				...
				if(killed==0) {
					...
					npc.say("Zapomniałeś? Miałeś wybrać się w góry i pokonać " + monsters + "!");
					return;
				}
				if(killed < killsnumber) {
	...
	public void addToWorld() {
		...
		fillQuestInfo(
				"Górskie Krasnoludy",
				"Górskie Krasnale regularnie niszczą tunele górników Semos. Poszukują śmiałka który się z nimi rozprawi.",
				true);
		step_1();
	}
	...
	public boolean isRepeatable(final Player player) {
 	...
 	public List<String> getHistory(final Player player) {
		...
		if(player.getQuest(QUEST_SLOT, 0).equals("start")) {
			...
			history.add("Frank poprosił mnie o wybranie się w góry i zabicie grupy "+
					givenNumber+" "+
			...
			if(new KilledInSumForQuestCondition(QUEST_SLOT, 2, givenNumber).fire(player, null, null)) {
				history.add("Zabiłem wystaczająco górskich krasnoludów, aby dostać moją nagrodę.");
			} else {
		...
		if(isCompleted(player)) {
			history.add("Pomogłem górnikom Semos i w zamian otrzymałem kilka kamieni szlachetnych z kopalni.");
		}	
		if (isRepeatable(player)) {
			history.add("Górnicy Semos znowu potrzebują pomocy.");
		} 
		int repetitions = player.getNumberOfRepetitions(getSlotName(), 3);
		if (repetitions > 0) {
			history.add("Pokonałem grupę "
					+ Grammar.quantityplnoun(repetitions, "") + " dla górników w Semos.");
		}
		return history; 
 	}
