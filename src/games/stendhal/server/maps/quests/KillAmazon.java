/* $Id: KillAmazon.java,v 1.32 2020/12/07 17:21:19 davvids Exp $ */
/***************************************************************************
 ...
 */
 ...
 public class KillAmazon extends AbstractQuest {

	private static final String QUEST_NPC = "Cal";
	private static final String QUEST_SLOT = "kill_amazon";
	private static final int delay = MathHelper.MINUTES_IN_ONE_WEEK*8;
	...
	public KillAmazon() {
		...
		enemyForces.put("amazonek",
				new Pair<Integer, String>(800,"Zapomniałeś co mi obiecałeś? Miałeś wybrać się tunelami na Amazońską wyspe i pozabijać amazonki!"));

		enemys.put("amazonek",
				Arrays.asList("amazonka łucznik",
							  "amazonka łowca",
							  "amazonka strażniczka wybrzeża",
							  "amazonka łucznik komandor",
							  "amazonka ochroniarz",
							  "amazonka mistrzyni strażniczek",
							  "amazonka komandor",
							  "amazonka zwiadowca",
							  "amazonka imperator"));
	...
	class GiveQuestAction implements ChatAction {
		...
		public void fire(final Player player, final Sentence sentence, final EventRaiser speakerNPC) {
			...
			speakerNPC.say("Daleko... daleko stąd zlokalizowana jest tropikalna wyspa. Znajdują się na niej Amazonki. Poszukuje kogoś kto odnajdzie tą wyspę i zabije grupę " + monstersType +
	...
	class RewardPlayerAction implements ChatAction {
		...
		public void fire(final Player player, final Sentence sentence, final EventRaiser speakerNPC) {
			...
			int moneyreward = 69*Rand.roll1D6();
			if(killed == killsnumber) {
				// player killed no more no less then needed soldiers
				speakerNPC.say("Świetnie. Przyjmij te wielki eliksiry w ramach nagrody. Myśle, że za tydzień będe miał dla ciebie kolejne zadanie.");
			} else {
				// player killed more then needed soldiers
				speakerNPC.say("Wybawiciel! Zabiłeś "+(killed-killsnumber)+" więcej "+
						Grammar.plnoun(killed-killsnumber, "")+"! Świetnie. Przyjmij te wielkie eliksiry w ramach nagrody. Myśle, że za tydzień będe miał dla ciebie kolejne zadanie.");
			}
			int karmabonus = 566*(1*killed/(killsnumber)-1);
			final StackableItem money = (StackableItem)
					SingletonRepository.getEntityManager().getItem("wielki eliksir");
	...
	class ExplainAction implements ChatAction {
		...
		public void fire(Player player, Sentence sentence, EventRaiser npc) {
				...
				if(killed==0) {
					...
					npc.say("Zapomniałeś? Miałeś odnaleźć wyspę i pokonać grupę " + monsters + "!");
					return;
				}
				if(killed < killsnumber) {
			...
			final String enemy = creatures.get(i);
		      ...
		      npc.add(ConversationStates.ATTENDING,
		    		  ...
		    		  new MultipleActions(
							  ...
							  new EquipItemAction("zwój amazon", 10),
							  new EquipItemAction("bełt", 5000),
							  new EquipItemAction("bełt stalowy", 3000),
							  new EquipItemAction("bełt złoty", 2000),
							  new EquipItemAction("bełt płonący", 1000),
							  ...
							  new EquipItemAction("skrzynia skarbów VII", 1),
							  new EquipItemAction("skrzynia skarbów VII", 1),
							  new EquipItemAction("skrzynia skarbów VIII", 1),
							  new EquipItemAction("skrzynia skarbów VIII", 1),
							  new EquipItemAction("skrzynia skarbów VIII", 1),
							  new EquipItemAction("skrzynia skarbów VIII", 1),
							  new EquipItemAction("skrzynia skarbów VIII", 1),
	...
	public void addToWorld() {
		...
		fillQuestInfo(
				"Wyspa Amazonek",
				"Cal poszukuje kogoś kto odnajdzie wyspę i pokona armię Amazonek.",
				true);
		step_1();
	}
	...
	public boolean isRepeatable(final Player player) {
 	...
 	public List<String> getHistory(final Player player) {
		...
		if(player.getQuest(QUEST_SLOT, 0).equals("start")) {
			...
			history.add("Cal poprosił mnie o odnalezienie wyspy i zabicie armii "+
					givenNumber+" "+
			...
			if(new KilledInSumForQuestCondition(QUEST_SLOT, 2, givenNumber).fire(player, null, null)) {
				history.add("Zabiłem wystaczająco Amazonek, aby dostać moją nagrodę.");
			} else {
		...
		if(isCompleted(player)) {
			history.add("Pomogłem Calowi i zabiłem Amazonki na wyspie.");
		}	
		if (isRepeatable(player)) {
			history.add("Cal w muzeum Kirdneh znowu potrzebuje pomocy.");
		} 
		int repetitions = player.getNumberOfRepetitions(getSlotName(), 3);
		if (repetitions > 0) {
			history.add("Pokonałem całą armię "
					+ Grammar.quantityplnoun(repetitions, "") + " dla Cala.");
		}
		return history; 
 	}
