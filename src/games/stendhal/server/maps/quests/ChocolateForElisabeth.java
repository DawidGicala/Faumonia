...
public class ChocolateForElisabeth extends AbstractQuest {
	...
	private void chocolateStep() {
		...
		npc.add(ConversationStates.ATTENDING,
				ConversationPhrases.QUEST_MESSAGES, 
				new AndCondition(new QuestStateStartsWithCondition(QUEST_SLOT, "done;"), new NotCondition(new TimePassedCondition(QUEST_SLOT, 1, REQUIRED_MINUTES))),
				ConversationStates.ATTENDING, 
		...
		npc.add(ConversationStates.ATTENDING,
				ConversationPhrases.QUEST_MESSAGES, 
				new AndCondition(new QuestActiveCondition(QUEST_SLOT), new NotCondition(new QuestStateStartsWithCondition(QUEST_SLOT, "done;"))),
				ConversationStates.ATTENDING,	
		...
		reward.add(new IncreaseAtkXPAction(200));
		reward.add(new IncreaseDefXPAction(250));
	...
	public List<String> getHistory(final Player player) {
		...
		if (player.isQuestInState(QUEST_SLOT, "start","mummy") || isCompleted(player)) {
			res.add("Mała Elisabeth potrzebuje tabliczkę czekolady. Podobno moge ją kupić w stolicy.");
		}
		if (player.isQuestInState(QUEST_SLOT, "start","mummy") && player.isEquipped("tabliczka czekolady") || isCompleted(player)) {
			res.add("Znalazłem pyszną tabliczkę czekolady dla Elisabeth. Musze porozmawiać z jej matką i poprosić o zgodę.");
		}
        ...
        if (isCompleted(player)) {
            if (isRepeatable(player)) {
                ...
                res.add("Elisabeth je czekoladę, którą jej dałem i dała mi w zamian kwiaty.	W nagrodę za ukończenie zadania otrzymałem: 500 pkt doświadczenia, 250 pkt ataku, 200 pkt obrony i 10 pkt karmy.");
            }			
		}
		return res;
	}
