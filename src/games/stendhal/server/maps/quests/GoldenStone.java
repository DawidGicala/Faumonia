...
public class GoldenStone extends AbstractQuest {
	...
	private static final int REQUIRED_WOOD = 10000;
	...
	public List<String> getHistory(final Player player) {
		...
		if ("rejected".equals(questState)) {
			res.add("Nie chcę zwoju od Jacera");
			return res;
		}
		res.add("Chcę zwój od Jacera");
		if ((player.isEquipped("money", REQUIRED_WOOD)) || isCompleted(player)) {
			res.add("Musze przynieść pieniadze.");
		}
		if (isCompleted(player)) {
			res.add("Dałem Jacerowi pieniądze. Dostałem przepiękny zwój");
		}
		if(isRepeatable(player)){
			res.add("Mogę iść po nowy zwój do Jacera.");
		} 
		return res;
	}
	...
	private void prepareRequestingStep() {
		...
		npc.add(ConversationStates.IDLE, 
			...
			"Witaj znowu! Masz te 10000 money o których wcześniej rozmawialiśmy?",
			null);
		...
		npc.add(ConversationStates.IDLE, 
			...
			"Dobrze, że jesteś. Przynieś szybko te 10000 money a dostaniesz zwój!",
			null);
		...
		npc.add(ConversationStates.ATTENDING,
			...
			ConversationStates.ATTENDING,
			"Już się umówiliśmy, że dotarczysz pieniądze. 10000 money, pamiętasz?",
			null);
		...
		npc.add(ConversationStates.ATTENDING,
				...
				"Mam przepiękny zwój. Moge ci go dać ale nie ma nic za darmo. Przynieś mi 10000 money a dam ci zwój. Co ty na to ?",
				null);
		...
		npc.add(ConversationStates.ATTENDING,
				...
				ConversationStates.QUEST_OFFERED, 
				"Mam kolejny zwój. Dam ci go za kolejne 10000 money. Pasuje?",
				null);
		...
		npc.add(ConversationStates.ATTENDING,
				...
				ConversationStates.ATTENDING, 
				null,
				new SayTimeRemainingAction(QUEST_SLOT,REQUIRED_MINUTES,"Dzięki za pieniądze. W sprawie kolejnego zwoju porozmawiamy za"));
	...
	private void prepareBringingStep() {
		...
		reward.add(new EquipItemAction("czarny zwój", 1));
	...
	public void addToWorld() {
		...
		fillQuestInfo(
				"Czarny zwój", 
				"Jacer da mi czarny zwój za trochę złota.", 
				true);
