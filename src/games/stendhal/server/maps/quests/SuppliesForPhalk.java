 ...
 */
 ...
 public class SuppliesForPhalk extends AbstractQuest {
	...
	final SpeakerNPC npc = npcs.get("Wrvil");
		...
		final List<ChatAction> actions = new LinkedList<ChatAction>();
		actions.add(new IncreaseXPAction(2000));
		...
		npc.add(ConversationStates.ATTENDING, "Phalk",
				...
				ConversationStates.QUEST_ITEM_QUESTION, 
				"Weź ten płaszcz, który dałem ci dla Phalka. Jeżeli zgubisz go to w zamian będziesz musiał zapłacić 10.000 money. Czy chcesz kupić nowy?",
				null);
	...
	private void getArmor() {
		...
		npc.add(ConversationStates.QUEST_ITEM_QUESTION, Arrays.asList("payment", "zapłatę"),
				new AndCondition(new QuestInStateCondition(QUEST_SLOT, 0, "clothes"),new QuestInStateCondition(QUEST_SLOT, 2, "none")),
				ConversationStates.QUEST_ITEM_QUESTION, 
				"Cóż.. zbroja będzie kosztować 10 sztabek żelaza. Masz je?",
				null);	
		...
		npc.add(ConversationStates.ATTENDING, Arrays.asList("payment", "zapłatę"),
				new AndCondition(new QuestInStateCondition(QUEST_SLOT, 0, "clothes"),new QuestInStateCondition(QUEST_SLOT, 2, "none")),
				ConversationStates.QUEST_ITEM_QUESTION, 
				"Zbroja będzie kosztować 10 sztabek żelaza. Masz je?",
				null);	
		...
		actions.add(new DropItemAction("żelazo",10));
		actions.add(new ChatAction() {
			public void fire(final Player player, final Sentence sentence, final EventRaiser npc) {
				final Item armor = SingletonRepository.getEntityManager().getItem("zbroja krasnoludzka");
				armor.setInfoString("Phalk");
				armor.setDescription("Oto błyszcząca zbroja z imieniem 'Phalk' wygrawerowanym w środku.");
				// remember the description
		...
		npc.add(ConversationStates.QUEST_ITEM_QUESTION, ConversationPhrases.YES_MESSAGES,
				new AndCondition(
						new AndCondition(new QuestInStateCondition(QUEST_SLOT, 0, "clothes"),new QuestInStateCondition(QUEST_SLOT, 2, "none")),
						new PlayerHasItemWithHimCondition("żelazo",10)),
				ConversationStates.ATTENDING, 
				"Dobrze, trzymaj.",
				new MultipleActions(actions)
		);
		...
		npc.add(ConversationStates.QUEST_ITEM_QUESTION, ConversationPhrases.YES_MESSAGES,
				new AndCondition(new QuestInStateCondition(QUEST_SLOT, 0, "clothes"),new QuestInStateCondition(QUEST_SLOT, 2, "none"),
				new NotCondition(new PlayerHasItemWithHimCondition("żelazo",10))),
				ConversationStates.ATTENDING, 
		...
		npc.add(ConversationStates.ATTENDING, "Phalk",
				...
				ConversationStates.QUEST_ITEM_QUESTION, 
				"Weź tą zbroję i daj ją Phalkowi. Jeżeli zgubisz ją to w zamian będziesz msuiał zapłacić 10.000 money. Czy chcesz zapłacić za zamianę z Phalkiem?",
				null);
		...
		actions2.add(new ChatAction() {
			public void fire(final Player player, final Sentence sentence, final EventRaiser npc) {
				final Item armor = SingletonRepository.getEntityManager().getItem("zbroja krasnoludzka");
				armor.setInfoString("Phalk");
				armor.setDescription("Oto lśniąca zbroja krasnoludzka z nazwą 'Phalk' wyrytą na niej.");
				// remember the description
				armor.setPersistent(true);
				armor.setBoundTo(player.getName());
				player.equipOrPutOnGround(armor);
	...
	final SpeakerNPC npc = npcs.get("Phalk");	
		...
		final List<ChatAction> actions = new LinkedList<ChatAction>();
		actions.add(new IncreaseXPAction(50000));
		actions.add(new IncreaseAtkXPAction(20000));
		actions.add(new IncreaseDefXPAction(10000));
		actions.add(new DropInfostringItemAction("zbroja krasnoludzka","Phalk"));
		...
		actions.add(new EquipItemAction("zbroja phalka", 1, true));
		...
		npc.add(ConversationStates.ATTENDING, Arrays.asList("clothes", "ubrania"),
				...
				new PlayerHasInfostringItemWithHimCondition("zbroja krasnoludzka","Phalk"),
	...
	public void addToWorld() {
		...
		fillQuestInfo(
				"Zapasy dla Phalka",
				"Phalk strażnik krasnali w kopalni Semos potrzebuje zapasów.",
				false);
		askForFood();
		receiveFood();
		getCloak();
	...
	public List<String> getHistory(final Player player) {
			...
			res.add("Rozmawiałem z Phalkiem, który pilnuje przejścia w kopalni Semos.");
			res.add("Phalk poprosił mnie, żebym przyniósł mu 3 kanapki, 3 soki z chmielu i 3 napoje z winogron. O kanapki powinienem pytać w piekarni, napoje mogę kupić w tawernie.");
			...
			res.add("Teraz Phalk potrzebuje płaszcza od Wrvil mieszkającego w mieście Wofol i zbroje od Mrotho, znajdującego się barakach Ados.");
			if (questState.startsWith("clothes")) {
				...
				if(new QuestInStateCondition(QUEST_SLOT, 2, "armor").fire(player,null, null)){
					res.add("Mrotho dał mi zbroie dla Phalka, ale musiałem pokryć dług.");
				}
				return res;
			} 
			res.add("Oddałem Phalkowi ekwipunek w nagrodę dostałem jego zbroje!	W nagrodę za ukończenie zadania otrzymałem: 50.000 pkt doświadczenia, 20.000 pkt ataku, 10.000 pkt obrony i zbroję phalka.");
