/* $Id: KillSedah.java,v 1.32 2020/12/06 20:11:19 davvids Exp $ */
/***************************************************************************
 ...
 */
 ...
 public class KillSedah extends AbstractQuest {

	private static final String QUEST_NPC = "Jin";
	private static final String QUEST_SLOT = "kill_sedah";
	...
	public KillSedah() {
		...
		enemyForces.put("imperialnych wojowników",
				new Pair<Integer, String>(1500,"Zapomniałeś co mi obiecałeś? Miałeś pokonać armie wojowników imperium. Znajdziesz je na południe od miasta."));

		enemys.put("imperialnych wojowników",
	...
	class GiveQuestAction implements ChatAction {
		...
		public void fire(final Player player, final Sentence sentence, final EventRaiser speakerNPC) {
			...
			speakerNPC.say("Prawde powiedziawszy, przybyłem tutaj w celach służbowych. Nie mów o tym nikomu, poszukuje najemników aby rozprawić się z oddziałami  " + monstersType +
					" na południu. Zabij co najmniej " + enemyForces.get(monstersType).first()+
					" "+ monstersType +
					" a wynagrodzę cię.");
	...
	class RewardPlayerAction implements ChatAction {
		...
		public void fire(final Player player, final Sentence sentence, final EventRaiser speakerNPC) {
			...
			int moneyreward = 82*Rand.roll1D6();
			if(killed == killsnumber) {
				// player killed no more no less then needed soldiers
				speakerNPC.say("Dobra robota! Weź tę wielkie eliksiry jako zapłatę... Myśle, że za tydzień będe miał dla ciebie kolejne zadanie.");
			} else {
				// player killed more then needed soldiers
				speakerNPC.say("Bardzo dobrze! Zabiłeś "+(killed-killsnumber)+" extra "+
						Grammar.plnoun(killed-killsnumber, "soldier")+"! Weź tę wielkie eliksiry jako zapłątę. Myśle, że za tydzień będe miał dla ciebie kolejne zadanie.");
			}
			...
			final String enemy = creatures.get(i);
		      ...
		      npc.add(ConversationStates.ATTENDING,
		    		  ...
		    		  new MultipleActions(
							  ...
							  new IncreaseAtkXPAction(60000),
							  new IncreaseDefXPAction(80000),
	...
	public void addToWorld() {
		...
		fillQuestInfo(
				"Misja wojskowa",
				"Jin, wynajmujący apartament w tawernie w Fado poszukuje najemników.",
				true);
		step_1();
	}
	...
	public boolean isRepeatable(final Player player) {
 	...
 	public List<String> getHistory(final Player player) {
		...
		if(player.getQuest(QUEST_SLOT, 0).equals("start")) {
			...
			history.add("Jin poprosiła mnie o zabicie armii "+
					givenNumber+" "+
			...
			if(new KilledInSumForQuestCondition(QUEST_SLOT, 2, givenNumber).fire(player, null, null)) {
				history.add("Zabiłem wystaczająco wojowników imperium, aby dostać moją nagrodę.");
			} else {
		...
		if(isCompleted(player)) {
			history.add("Pomogłem Jinowi i otrzymałem w zamian zapas wielkich eliksirów!");
		}	
		if (isRepeatable(player)) {
			history.add("Jin znowu potrzebują pomocy.");
		} 
		int repetitions = player.getNumberOfRepetitions(getSlotName(), 3);
		if (repetitions > 0) {
			history.add("Pokonałem armię "
					+ Grammar.quantityplnoun(repetitions, "") + " razy dla Jina.");
		}
		return history; 
 	}
