/* $Id: SkillerTask3.java,v 1.46 2020/11/29 14:26:42 davvids Exp $ */
/***************************************************************************
...
import games.stendhal.server.entity.item.scroll.SkillerTask3Scroll;
 ...
 * </ul>
 */
public class SkillerTask3 extends AbstractQuest {

	private static final int REQUIRED_LEVEL = 1;

	private static final int REQUIRED_MONEY = 1000;

	private static final int REQUIRED_MINUTES = 1 * 0;

	private static final String QUEST_SLOT = "skiller_nr_3";
	...
	private void step_1() {
		final SpeakerNPC npc = npcs.get("Lynus");
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
			...
			"Witaj jestem jednym z tutejszych trenerów. #Handluje specjalnymi biletami wstępu na arenę treningową..", null);
		...
		npc.add(
			...
			"Witaj, wróciłeś po kolejny bilet na arene treningową?", null);
		...
		npc.add(
			...
			null,
			new SayTimeRemainingAction(QUEST_SLOT, 1, REQUIRED_MINUTES, "Wszystko w porządku? Mam nadzieję, że nie chcesz kolejnego biletu na arenę treningową. Nie moge ci go dać przez co najmniej"));
		...
		npc.add(ConversationStates.INFORMATION_1, 
			...
			ConversationStates.QUEST_OFFERED, 
			"Chcesz trochę poćwiczyć? Handluję biletami na arenę treningową. To będzie Cię kosztować "
								+ REQUIRED_MONEY
								+ " money. Chcesz kupić?",
			null);
		...
		npc.add(ConversationStates.QUEST_OFFERED,
				...
				ConversationStates.ATTENDING, 
				"W porządku oto twój bilet na arenę treningową. Gdy go użyjesz to wrócisz za około 8 godzin. Jeśli będziesz chciał wcześniej wrócić to skorzystaj ze schodów.",
				new MultipleActions(
						new DropItemAction("money", REQUIRED_MONEY),
						new EquipItemAction("bilet treningowy III", 1, true),
						new TeleportAction("int_semos_skill_house", 18, 16, Direction.LEFT),
	...
	public void addToWorld() {
		...
		SingletonRepository.getLoginNotifier().addListener(new LoginListener() {
			public void onLoggedIn(final Player player) {
			    SkillerTask3Scroll scroll = (SkillerTask3Scroll) SingletonRepository.getEntityManager().getItem("bilet treningowy III");
