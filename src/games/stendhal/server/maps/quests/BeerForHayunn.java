...
public class BeerForHayunn extends AbstractQuest {
	...
	public List<String> getHistory(final Player player) {
		...
		res.add("Rozmawiałem z Hayunnem.");
		final String questState = player.getQuest(QUEST_SLOT);
		if ("rejected".equals(questState)) {
			res.add("Nie chcę dać soku z chmielu Hayunnowi.");
		}
		if (player.isQuestInState(QUEST_SLOT, "start", "done")) {
			res.add("Obiecałem dać Hayunnowi sok z chmielu.");
		}
		if (("start".equals(questState) && player.isEquipped("sok z chmielu"))
				|| "done".equals(questState)) {
			res.add("Mam sok z chmielu, powinienem przekazać go Hayunnowi.");
		}
		if ("done".equals(questState)) {
			res.add("Dałem sok z chmielu Hayunnowi. 	W nagrode za ukończenie zadania otrzymałem: 100 pkt doświadczenia, 10 pkt karmy i 500 money,");
		}
		return res;
	}
	...
	private void prepareBringingStep() {
		...
		npc.add(
			...
			new PlayerHasItemWithHimCondition("sok z chmielu"),
			ConversationStates.ATTENDING,
			"*gul gul* Ach! Trafiłeś. Trzymaj troche złota w podziece. Daj znać jeżeli będziesz czegoś potrzebował, dobrze?",
