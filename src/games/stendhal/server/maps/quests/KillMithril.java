/* $Id: KillMithril.java,v 1.32 2020/12/07 14:31:19 davvids Exp $ */
/***************************************************************************
 ...
 */
 ...
 public class KillMithril extends AbstractQuest {

	private static final String QUEST_NPC = "Banio";
	private static final String QUEST_SLOT = "kill_mithril";
	...
	public KillMithril() {
		...
		enemyForces.put("żołnierzy mithrilu",
				new Pair<Integer, String>(10000,"Zapomniałeś co mi obiecałeś? Miałeś wybrać się na północ do zamku żołnierzy mithrilu i pokonać wojowników!"));

		enemys.put("żołnierzy mithrilu",
				Arrays.asList("mithrilbourgh żołnierz",
							  "mithrilbourgh łucznik",
							  "rycerz mithrilbourgh",
							  "elitarny łucznik mithrilbourgh",
							  "sierżant mithrilbourgh",
							  "porucznik mithrilbourgh",
							  "pułkownik mithrilbourgh",
							  "czarodziej mithrilbourgh",
							  "mithrilbourgh generał"));
	...
	class GiveQuestAction implements ChatAction {
		...
		public void fire(final Player player, final Sentence sentence, final EventRaiser speakerNPC) {
			...
			speakerNPC.say("Na północ od naszej stolicy, w górach zlokalizowany jest obóz wrogich żołnierzy. Nasz burmistrz ignoruje zagrożenie ale my - mieszkańcy nie chcemy ryzykować. W imieniu wszystkich mieszkańcow Ados poszukuje kogoś kto wybierze się w góry i zabije  " + monstersType +
	...
	class RewardPlayerAction implements ChatAction {
		...
		public void fire(final Player player, final Sentence sentence, final EventRaiser speakerNPC) {
			...
			int moneyreward = 9*Rand.roll1D6();
			if(killed == killsnumber) {
				// player killed no more no less then needed soldiers
				speakerNPC.say("Wybawiciel! Większość pieniędzy przeznaczamy na zakup zapasów w razie ewentualnej wojny... nasi rybacy gotują przepyszną zupę rybną, weź ją mam nadzieje, że ci posmakuje... Myśle, że za tydzień będe miał dla ciebie kolejne zadanie.");
			} else {
				// player killed more then needed soldiers
				speakerNPC.say("Wybawiciel! Zabiłeś "+(killed-killsnumber)+" więcej "+
						Grammar.plnoun(killed-killsnumber, "")+"! Większość pieniędzy przeznaczamy na zakup zapasów w razie ewentualnej wojny... nasi rybacy gotują przepyszną zupę rybną, weź ją, mam nadzieje, że ci posmakuje. Myśle, że za tydzień będe miał dla ciebie kolejne zadanie.");
			}
			int karmabonus = 1*(1*killed/(killsnumber)-1);
			final StackableItem money = (StackableItem)
					SingletonRepository.getEntityManager().getItem("zupa rybna");
	...
	class ExplainAction implements ChatAction {
		...
		public void fire(Player player, Sentence sentence, EventRaiser npc) {
				...
				if(killed==0) {
					...
					npc.say("Zapomniałeś? Miałeś wybrać się na północ i pokonać " + monsters + "!");
					return;
				}
				if(killed < killsnumber) {
	...
	public void addToWorld() {
		...
		fillQuestInfo(
				"Zamek Mithrili",
				"Mieszkańcy Ados obawiają się najazdu! Poszukują kogoś kto wybierze się na północ i zabije żołnierzy mithrilu.",
				true);
		step_1();
	}
	...
	public boolean isRepeatable(final Player player) {
 	...
 	public List<String> getHistory(final Player player) {
		...
		if(player.getQuest(QUEST_SLOT, 0).equals("start")) {
			...
			history.add("Banio poprosił mnie o wybranie się na północ i zabicie armii "+
					givenNumber+" "+
			...
			if(new KilledInSumForQuestCondition(QUEST_SLOT, 2, givenNumber).fire(player, null, null)) {
				history.add("Zabiłem wystaczająco wojowników mithrilu, aby dostać moją nagrodę.");
			} else {
		...
		if(isCompleted(player)) {
			history.add("Pomogłem mieszkańcom Ados w zamian otrzymałem zapas zup rybnych.");
		}	
		if (isRepeatable(player)) {
			history.add("Mieszkańcy Ados znowu potrzebują pomocy.");
		} 
		int repetitions = player.getNumberOfRepetitions(getSlotName(), 3);
		if (repetitions > 0) {
			history.add("Pokonałem całą armię "
					+ Grammar.quantityplnoun(repetitions, "") + " dla mieszkańców Ados.");
		}
		return history; 
 	}
