/* $Id: ScrollDeath.java,v 1.46 2012/04/19 18:26:42 kymara Exp $ */
/***************************************************************************
...
import games.stendhal.server.entity.item.scroll.DeathScroll;
 ...
 * PARTICIPANTS:
 * <ul>
 * <li>SmierciowyNPC, a dealer in bilet śmierci
 * </ul>
 *
 * STEPS:
 * <ul>
 * <li>The NPC sells bilet śmierci to players above level 300</li>
 ...
 * </ul>
 */
public class ScrollDeath extends AbstractQuest {

	private static final int REQUIRED_LEVEL = 300;

	private static final int REQUIRED_MONEY = 999999999;

	private static final int REQUIRED_MINUTES = 60 * 60 * 60;

	private static final String QUEST_SLOT = "smierciowy_bilet";
	...
	private void step_1() {
		final SpeakerNPC npc = npcs.get("SmierciowyNPC");
		...
		npc.add(
			...
			"Wróciłeś po kolejny bilet śmierci?", null);
		...
		npc.add(ConversationStates.QUEST_OFFERED,
				...
				ConversationStates.ATTENDING, 
				"W porządku oto twój bilet śmierci. Gdy użyjesz to wrócisz za około 3 godziny. Jeśli będziesz chciał wcześniej wrócić to skorzystaj tam stan na herbie Zakopanego, który zabierze Ciebie z powrotem.",
				new MultipleActions(
						new DropItemAction("money", REQUIRED_MONEY),
						new EquipItemAction("bilet śmierci", 1, true),
		...
		npc.add(ConversationStates.ATTENDING,
			Arrays.asList("deal", "ticket", "bilet śmierci", "yes", "tak"),
			new LevelGreaterThanCondition(REQUIRED_LEVEL-1),
			ConversationStates.ATTENDING,
	...
	public void addToWorld() {
		...
		SingletonRepository.getLoginNotifier().addListener(new LoginListener() {
			public void onLoggedIn(final Player player) {
			    DeathScroll scroll = (DeathScroll) SingletonRepository.getEntityManager().getItem("bilet śmierci");
		...
		fillQuestInfo(
				"bilet śmierci",
				"bilet śmierci wysyła na wycieczkę do obcej krainy pokrytej piaskiem.",
				false);
		step_1();
