...
public class BlueStone extends AbstractQuest {
	...
	private static final int REQUIRED_WOOD = 2500;
	...
	public List<String> getHistory(final Player player) {
		...
		if ("rejected".equals(questState)) {
			res.add("Nie chcę zwoju od Pariri");
			return res;
		}
		res.add("Chcę zwój od Pariri");
		if ((player.isEquipped("money", REQUIRED_WOOD)) || isCompleted(player)) {
			res.add("Musze przynieść pieniadze.");
		}
		if (isCompleted(player)) {
			res.add("Dałem Pariri pieniądze. Dostałem przepiękny zwój");
		}
	...
	private void prepareRequestingStep() {
		...
		npc.add(ConversationStates.IDLE, 
			...
			"Witaj znowu! Masz te 2500 money o których wcześniej rozmawialiśmy?",
			null);
		...
		npc.add(ConversationStates.IDLE, 
			...
			"Dobrze, że jesteś. Przynieś szybko te 2500 money a dostaniesz zwój!",
			null);
		...
		npc.add(ConversationStates.ATTENDING,
			...
			ConversationStates.ATTENDING,
			"Już się umówiliśmy, że dotarczysz pieniądze. 2500 money, pamiętasz?",
			null);
		...
		npc.add(ConversationStates.ATTENDING,
				...
				"Mam przepiękny zwój. Moge ci go dać ale nie ma nic za darmo. Przynieś mi 2500 money a dam ci zwój. Co ty na to ?",
				null);
		...
		npc.add(ConversationStates.ATTENDING,
				...
				ConversationStates.QUEST_OFFERED, 
				"Mam kolejny zwój. Dam ci go za kolejne 2500 money. Pasuje?",
				null);
	...
	private void prepareBringingStep() {
		...
		reward.add(new EquipItemAction("niebieski zwój", 1));
	...
	public void addToWorld() {
		...
		fillQuestInfo(
				"Niebieski zwój", 
				"Pariri da mi niebieski zwój za trochę złota.", 
				true);
