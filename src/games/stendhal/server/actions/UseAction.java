...
public class UseAction implements ActionListener {
	 ...
	 * @return true if item is bound false otherwise
	 */
	protected boolean isItemMinLevel(final Player player, final RPObject object) {
		if (object instanceof Item) {
			final Item item = (Item) object;
			if (item.has("minlevel")
					&& player.getLevel() < item.getInt("minlevel")) {
					player.sendPrivateText("Nie jesteś wystarczająco doświadczony, aby użyć "
						+ item.getName() + ". Wymagany jest poziom " + item.getInt("minlevel"));
				return true;
			}
		}
		return false;
