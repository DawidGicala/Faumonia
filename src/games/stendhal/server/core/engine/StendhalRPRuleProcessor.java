/* $Id: StendhalRPRuleProcessor.java,v 1.94 2013/01/10 19:16:16 nhnb Exp $ */
/***************************************************************************
...
public class StendhalRPRuleProcessor implements IRPRuleProcessor {
	...
	private static String welcomeMessage = " Witaj w #FaumoniaOnline dzielny wojowniku! Odwiedź stronę #www.faumonia.pl";
	...
	public void setContext(final RPServerManager rpman) {
		try {
			...
			logger.info("Running FaumoniaOnline server VERSION " + Debug.VERSION + preRelease);
			Translate.init();
	...
	public boolean checkGameVersion(final String game, final String version) {
		try {
			if (!game.equals(Configuration.getConfiguration().get("server_typeGame", "FaumoniaOnline"))) {
