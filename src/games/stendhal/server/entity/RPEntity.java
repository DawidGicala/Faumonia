...
public abstract class RPEntity extends GuidedEntity {
	...
	private static final float MEDAL_DEF_MULTIPLIER = 1.0f;
	...
	public static void generateRPClass() {
		try {
			...
			entity.addRPSlot("medal", 1, Definition.PRIVATE);
			...
			entity.addRPSlot("keyring", 15, Definition.PRIVATE);
	...
	public void adddef_xp(final int newdef_xp) {
		...
		new GameEvent(getName(), "added def_xp",
params).raise();
...
System.out.printf("  drop: %2d %2d\n", attackerRoll, defenderRoll);
	...
	public boolean hasMedal() {
		return isEquippedItemClass("medal", "medal");
	}
	
	public Item getMedal() {
		return getEquippedItemClass("medal", "medal");
	}
	...
	public float getItemAtk() {
		...
		return weapon + armor + boots + cloak + glove + helmet + legs + necklace + ring + medal + shield;
	}
	...
	public float getItemDef() {
		...
		if (hasMedal()) {
			item = getMedal();
			medal= (int) (item.getDefense() / getItemLevelModifier(item));
		}
