/* $Id: GameScreen.java,v 1.223 2012/12/17 16:34:24 kiheru Exp $ */
/***************************************************************************
...
public class GameScreen extends JComponent implements IGameScreen, DropTarget,
	 ...
	 */
	private static final int PAN_SCALE = 12;
	/**
	 ...
	 */
	private static final int PAN_INERTIA = 35;
	/**
	 * Space at the right and bottom of the screen next to the off line
	 * indicator icon.
	 */
	private static final int OFFLINE_MARGIN = 15;
	...
	private void adjustView() {
		...
		if ((sx < 0) || (sx >= sw) || (sy < -SIZE_UNIT_PIXELS) || (sy > sh)) {
			...
			if (speed != 0) {
				 ...
				 */
				if (dvx < 0) {
					dx = Math.max(Math.min(-2, dx), dvx);
				} else if (dvx > 0) {
					dx = Math.min(Math.max(2, dx), dvx);
				}

				if (dvy < 0) {
					dy = Math.max(Math.min(-2, dy), dvy);
				} else if (dvy > 0) {
					dy = Math.min(Math.max(2, dy), dvy);
				}
